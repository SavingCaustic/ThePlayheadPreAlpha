...
class AudioLoggerBuffer {
  public:
    ...
    bool addAudioLog(int code, const std::string &message) {
        ...
        strncpy(newLog.message, message.c_str(), sizeof(newLog.message) - 1);
    ...
    // Non-blocking write
    bool write(const LoggerRec &logRec) {
        ...
        buffer[currentWriteIndex & bufferSizeMask] = logRec;
    ...
    bool read(LoggerRec &logRec) {
        ...
        logRec = buffer[currentReadIndex & bufferSizeMask];
